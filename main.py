...
def reformatDate(dictval):
    if dictval == 'None' or dictval == 'Null':
        ...
        for attr in [u'year', u'month', u'day', u'hour', u'minute', u'second']:
            ...
            try:
                simpledate[attr] = dictval[u'Date'][attr] if u'Date' in dictval.iterkeys() else dictval[attr]
            except KeyError, err:
                print 'error:: {} input date is {}'.format(err, dictval)
                simpledate[attr] = None
        if simpledate[u'hour'] or simpledate[u'minute'] or simpledate[u'second']:
            ...
            #myDate = "{}/{}/{} {}:{}:{}".format(simpledate['day'], simpledate['month'], simpledate['year'], simpledate['hour'], simpledate['minute'], simpledate['second'])
        #print "resulting date is {}".format(myDate)
    return myDate
...
def prepare_data():
    ...
    bookings_json = raw_call_wrapper(key=netsuite_key,
                                     ...
                                     passwd=session['password'], xml_str='''
                                                                                            ...
                                                                                            ''')
    # call Netsuite Openair and retrieve projecttasks info
    project_tasks_json = raw_call_wrapper(key=netsuite_key,
                                          username=session['username'],
                                          passwd=session['password'], xml_str='''
    ...
    # iterate through the project_tasks_json

    for project_task in project_tasks_json['response']['Read']['Projecttask']:
        project_id = project_task['projectid']
        task_id = project_task['id']
        bClosed = project_task['closed']
        up_dated = reformatDate(project_task['updated'])
        name = project_task['name']
        estimated_hours = project_task['estimated_hours']
        planned_hours = project_task['planned_hours']
        calc_finishes = project_task['calculated_finishes']
        calc_starts = project_task['calculated_starts']
        currency = project_task['currency']

        # update the projects_dict

        try:
            projects_dict[users_name][project_id]['tasks'][task_id]['name'] = name
            projects_dict[users_name][project_id]['tasks'][task_id]['updated'] = up_dated
            projects_dict[users_name][project_id]['tasks'][task_id]['estimated_hours'] = estimated_hours
            projects_dict[users_name][project_id]['tasks'][task_id]['planned_hours'] = planned_hours
            projects_dict[users_name][project_id]['tasks'][task_id]['calculated_starts'] = calc_starts
            projects_dict[users_name][project_id]['tasks'][task_id]['calculated_finishes'] = calc_finishes
            projects_dict[users_name][project_id]['tasks'][task_id]['currency'] = currency
            projects_dict[users_name][project_id]['tasks'][task_id]['closed'] = bClosed
        except KeyError:
            try:
                projects_dict[users_name][project_id]['tasks'][task_id] = {
                                                                            'name' : name,
                                                                            ...
                                                                            'calculated_starts' : calc_starts,
                                                                            'calculated_finishes' : calc_finishes,
                                                                            'currency' : currency,
                                                                            'closed' : bClosed
                                                                        }
            except KeyError:
