...
def prepare_dict():
    ...
    for tasks in tasks_json['response']['Read']['Task']:
        ...
        if project_id not in succinct_projects_tasks.keys():
            succinct_projects_tasks[project_id] = {}
        ...
        if project_id not in projects_dict.keys():
            projects_dict[project_id] = {}
        if 'tasks' not in projects_dict[project_id].keys():
            projects_dict[project_id]['tasks'] = {}
        if task_id not in projects_dict[project_id]['tasks'].keys():
            projects_dict[project_id]['tasks'][task_id] = {}
